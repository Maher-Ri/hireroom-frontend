...
import { useJobsApplied } from "@/hooks/queries/useApplication";
...
const JobsAppliedPage = () => {
  ...
  const {
    ...
    refetch,
    isRefetching,
  } = useJobsApplied(session?.jwt, applicationStatus);
        ...
        <>
          <div className="space-y-4 py-4">
            {applications?.map((application: Application) => (
              <JobCard
                companyLogoUrl={application?.job?.company.logo
                ...
                timePosted={moment(application?.createdAt).fromNow()} id={0} location={""}              />
            ))}
          </div>
